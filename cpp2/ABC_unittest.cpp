...
const static std::map<int, std::string> gAlg2Name = {
    {1, "ABC"}, {2, "EABC"}, {3, "HABC"}, {4, "OABC"}, {5, "mABC"}, {6, "MeanABC"}, {7, "NABC"}
};
...
private:
    ...
    int randomSolution(std::vector<int> &solution, std::vector<int>& rsolution)
    {
        ...
        std::set<double> forbidchk{0.0, 1.0};

        std::vector<double> c(FLAGS_SD, 0.0);
        std::set<double> forbidc{0.0, 0.25, 0.50, 0.75, 1.0};
        double miu = 4.0;
        ...
        for (int i = 0; i < solution.size(); ++i) {
            switch(FLAGS_algorithm_type) {
                ...
                case 5: // mABC
                    c[0] = random(0,1);
                    while(forbidc.count(c[0])) {
                        c[0] = random(0,1);
                    }
                    if (i < FLAGS_SD - 1) {
                        c[i+1] = miu * c[i] * (1.0 - c[i]);
                    }
                    solution[i]=FLAGS_lb+c[i]*(FLAGS_up-FLAGS_lb);
                    rsolution[i] = FLAGS_lb + FLAGS_up - solution[i];
                    break;
                default:
                    break;
            ...
            rsolution[i] = int(rsolution[i]);
            lbUpCheck(solution, i);
            lbUpCheck(rsolution, i);
    ...
    int initilize()
    {
        ...
        for (int foodIdx = 0; foodIdx < FLAGS_FoodNumber; ++foodIdx){
            /****蜜源的初始化*****/
            NectarSource[foodIdx].trueFit = calculationTruefit(NectarSource[foodIdx].solution);  
            ...
            /****采蜜蜂的初始化*****/
            EmployedBee[foodIdx] = NectarSource[foodIdx];
            /****观察蜂的初始化****/
            OnLooker[foodIdx] = NectarSource[foodIdx];
        }
        /*****最优蜜源的初始化*****/
    ...
    int topNSolution()
    {
        if (FLAGS_algorithm_type != 3 // HABC
            && FLAGS_algorithm_type != 4 // OABC
            && FLAGS_algorithm_type != 5) { // mABC
            return FICUS_SUCC;
        }
        struct Data {
            std::vector<int> solution;
        ...
        for (int i = 0; i < FLAGS_FoodNumber; ++i) {
            ...
            dataList[idx].trueFit = calculationTruefit(dataList[idx].solution);
    ...
    // 计算真实的函数值 f(x)
    double calculationTruefit(std::vector<int>& solution)
    {
        CHECK_ASSERT_RTN(solution.size() == FLAGS_SD);
    ...
    {
        int i,j,k,r1,r2,r3,r4,r5,maxIdx;
        int best = BestIndex;  
        ...
        for (i = 0;i < FLAGS_FoodNumber; ++i) {
            ...
            switch(FLAGS_algorithm_type){
                case 1: // ABC
                    ...
                    lbUpCheck(EmployedBee[i].solution, param2change);
                    EmployedBee[i].trueFit=calculationTruefit(EmployedBee[i].solution);
                    break;
                case 2: // EABC
                    filter.insert(i);
                    filter.insert(best);
                ...
                case 3: // HABC
                    ...
                    lbUpCheck(tempSource.solution, param2change);
                    fx1 = calculationTruefit(tempSource.solution);
                    ...
                    EmployedBee[i].solution[param2change]=NectarSource[i].solution[param2change]
                        +random(-1, 1)*(NectarSource[i].solution[param2change]-NectarSource[k].solution[param2change])
                        +random(0, 2)*(NectarSource[best].solution[param2change]-NectarSource[i].solution[param2change]);
                    lbUpCheck(EmployedBee[i].solution, param2change);
                    EmployedBee[i].trueFit = calculationTruefit(EmployedBee[i].solution);
                ...
                case 4: // OABC
                    while (1){
                        k=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
                        if (k != i){
                            break;
                        }
                    }
                    Rij=random(-1,1);
                    EmployedBee[i].solution[param2change]=NectarSource[i].solution[param2change]
                        +(1.0-0.6*iter/FLAGS_MCN)*Rij*(NectarSource[i].solution[param2change]-NectarSource[k].solution[param2change]);
                    lbUpCheck(EmployedBee[i].solution, param2change);
                    EmployedBee[i].trueFit = calculationTruefit(EmployedBee[i].solution);
                    break;
                case 5: // mABC
                    ...
                    for (int j = 0; j< FLAGS_SD; ++j) {
                        Rij=random(-1,1);
                        EmployedBee[i].solution[j]=Rij*NectarSource[i].solution[j]
                            +(1.0-Rij)*NectarSource[k].solution[j];
                        lbUpCheck(EmployedBee[i].solution, j);
                    }
                    EmployedBee[i].trueFit=calculationTruefit(EmployedBee[i].solution);
                    break;
                case 6: // MeanABC
                    ...
                    EmployedBee[i].solution[param2change]=NectarSource[i].solution[param2change]
                        +Rij*(NectarSource[i].solution[param2change]-NectarSource[k].solution[param2change])
                        +random(0,1.5)*(NectarSource[i].solution[param2change]*-1);
                    lbUpCheck(EmployedBee[i].solution, param2change);
                    EmployedBee[i].trueFit = calculationTruefit(EmployedBee[i].solution);
                    break;
                case 7: // NABC
                    getBestNeighbor(i, maxIdx);
                    Rij=random(-1,1);
                    EmployedBee[i].solution[param2change]=NectarSource[maxIdx].solution[param2change]
                        +Rij*(NectarSource[maxIdx].solution[param2change]-NectarSource[i].solution[param2change]);
                    lbUpCheck(EmployedBee[i].solution, param2change);
                    EmployedBee[i].trueFit = calculationTruefit(EmployedBee[i].solution);
                    break;
                default:
            ...
            if (EmployedBee[i].trueFit>NectarSource[i].trueFit) {
                NectarSource[i].solution = EmployedBee[i].solution;
    ...
    int getBestNeighbor(int idx, int& maxIdx)
    {
        int r;
        std::set<int> st;
        st.insert(idx);
        int count = 0;
        while(count < 5) {
            r=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
            while(st.count(r)){
                r=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
            }
            st.insert(r);
        ...
        maxIdx = *st.begin();
        double trueFit = NectarSource[maxIdx].trueFit;
        for (auto& jdx: st){
            if (jdx == maxIdx) continue;
            if (NectarSource[jdx].trueFit > trueFit) maxIdx = jdx; // 目标函数最大化
            {
                trueFit = NectarSource[jdx].trueFit;
                maxIdx = jdx;
            }
        }
        return FICUS_SUCC;
    ...
    int lbUpCheck(std::vector<int>& solution, int idx)
    {
        /*******判断是否越界*******/
        if (solution[idx]<FLAGS_lb) {
            solution[idx]=FLAGS_lb;
        }
        if (solution[idx]>=FLAGS_up) {
            solution[idx]=FLAGS_up;
        }
        return FICUS_SUCC;
    }
        ...
        while(t<FLAGS_FoodNumber) {
            ...
            if(R_choosed<NectarSource[i].rfitness) {
                ...
                switch(FLAGS_algorithm_type){
                    case 1: // ABC
                        ...
                        OnLooker[i].solution[param2change]=NectarSource[i].solution[param2change]
                            +Rij*(NectarSource[i].solution[param2change]-NectarSource[k].solution[param2change]);
                        lbUpCheck(OnLooker[i].solution, param2change);
                        OnLooker[i].trueFit=calculationTruefit(OnLooker[i].solution);
                        break;
                    case 2: // EABC
                        filter.insert(i);
                        filter.insert(best);
                    ...
                    case 3: // HABC
                        ...
                        if (random(0, 1) < cr){
                            ...
                            OnLooker[i].solution[param2change]=NectarSource[best].solution[param2change]
                                +random(-1, 1)*(NectarSource[r1].solution[param2change]-NectarSource[r2].solution[param2change]);
                            lbUpCheck(OnLooker[i].solution, param2change);
                            OnLooker[i].trueFit = calculationTruefit(OnLooker[i].solution);
                        }
                        break;
                    ...
                    case 5: // mABC
                        while (1){
                            r1=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
                            r2=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
                            r3=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
                            if (r1 != r2 && r1 != r3) {
                                break;
                            }
                        }
                        Rij=random(-1,1);
                        OnLooker[i].solution[param2change]=NectarSource[i].solution[param2change]
                            +Rij*(NectarSource[best].solution[param2change]-NectarSource[r1].solution[param2change])
                            +(1.0-Rij)*(NectarSource[r2].solution[param2change]-NectarSource[r3].solution[param2change]);
                        lbUpCheck(OnLooker[i].solution, param2change);
                        OnLooker[i].trueFit=calculationTruefit(OnLooker[i].solution);
                        break;
                    case 6: // MeanABC
                    ...
                    case 7: // NABC
                        getBestNeighbor(i, maxIdx);
                        Rij=random(-1,1);
                        OnLooker[i].solution[param2change]=NectarSource[maxIdx].solution[param2change]
                            +Rij*(NectarSource[maxIdx].solution[param2change]-NectarSource[i].solution[param2change]);
                        lbUpCheck(OnLooker[i].solution, param2change);
                        OnLooker[i].trueFit = calculationTruefit(OnLooker[i].solution);
                        break;
                ...
                if (OnLooker[i].trueFit>NectarSource[i].trueFit) {
                    NectarSource[i].solution=OnLooker[i].solution;
                    NectarSource[i].trail=0;
            ...
            switch(FLAGS_algorithm_type){
                ...
                case 6: // MeanABC
                    for (int j = 0; j< FLAGS_SD; ++j) {
                        ...
                        solution[j] = int(solution[j]);
                        lbUpCheck(solution, j);
                    }
                    NectarSource[i].trail=0;
                    NectarSource[i].trueFit=calculationTruefit(solution);
                    break;
                case 3: // HABC
                ...
                case 4: // OABC
                    for (int j = 0; j< FLAGS_SD; ++j) {
                        R=random(0,1);
                        solution[j]=FLAGS_lb+R*(FLAGS_up-FLAGS_lb)
                            +(1.0-R)*(solution[j]-BestSource.solution[j]);
                        solution[j] = int(solution[j]);
                        lbUpCheck(solution, j);
                    }
                    NectarSource[i].trail=0;
                ...
                case 7: // NABC
                    while (1){
                        a=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
                        b=(int)random(0,FLAGS_FoodNumber) % FLAGS_FoodNumber;
                        if(a != i && b!= i && a != b) {
                            break;
                        }
                    }
                    ...
                    for (int j = 0; j< FLAGS_SD; ++j) {
                        R=random(0,1);
                        solution[j]=0.3*solution[j] + 0.3*BestSource.solution[j]
                            + 0.3*(NectarSource[a].solution[j]-NectarSource[b].solution[j]);
                        solution[j] = int(solution[j]);
                        lbUpCheck(solution, j);
                    }
                    NectarSource[i].trail=0;
    ...
    int sfla()
    {
        ...
        for (int k = 0; k < cnt; ++k) {
            ...
            tempSource.trueFit = calculationTruefit(tempSource.solution);
            ...
            if (tempSource.trueFit>worst.trueFit) { // 子群最好的
                ...
                if (tempSource.trueFit>worst.trueFit) { // 子群最好的
                    ...
                    worst.trueFit = calculationTruefit(worst.solution);
