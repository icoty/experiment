    ...
    int start()
    {
        ...
        while(iter < FLAGS_MCN) {
            ...
            if (iter == FLAGS_MCN) {
                ...
                double actualFx = calculationTruefit(BestSource.solution);
...
private:
    ...
    // 计算真实的函数值 f(x)
    double calculationTruefit(std::vector<double>& solution)
    {
        CHECK_ASSERT_RTN(solution.size() == FLAGS_SD);
    ...
    int getBestNeighbor(int idx, int& maxIdx)
    {
        ...
        for (auto& jdx: st){
            ...
            if (NectarSource[jdx].trueFit < trueFit) // 目标函数最小化
            {
    ...
    int lbUpCheck(std::vector<double>& solution, int idx)
    {
    ...
    int sendScoutBees()    
    {
        ...
        for (int i = 0; i < FLAGS_FoodNumber; ++i) {
            ...
            switch(FLAGS_algorithm_type){
                ...
                case 6: // MeanABC
                    ...
                    NectarSource[i].trueFit=calculationTruefit(NectarSource[i].solution);
                    break;
                case 3: // HABC
                ...
                case 4: // OABC
                    for (int j = 0; j< FLAGS_SD; ++j) {
                        R=random(0,1);
                        NectarSource[i].solution[j]=FLAGS_lb+R*(FLAGS_up-FLAGS_lb)
                            +(1.0-R)*(NectarSource[i].solution[j]-BestSource.solution[j]);
                    }
                    NectarSource[i].trail=0;
                    NectarSource[i].trueFit=calculationTruefit(NectarSource[i].solution);
                    break;
        ...
        if (FLAGS_algorithm_type == 3) { // HABC 混合蛙跳算法
            for (int i = 0; i < 10; ++i){
                sfla();
            }
        }
