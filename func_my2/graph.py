...
# ws dream 数据集: https://github.com/wsdream/wsdream-dataset
import matplotlib
...
# https://www.cnblogs.com/techflow/p/13772751.html
...
with open(file_name, "r") as ouf:
    ...
    if "HABC" in dump_data:
        fitness3 = dump_data["HABC"]['fitness']
        plt.plot(xf,fitness3,'--',color = 'y',label="HABC")

    if "OABC" in dump_data:
        fitness4 = dump_data["OABC"]['fitness']
        plt.plot(xf,fitness4,':',color = 'b',label="OABC")

    if "mABC" in dump_data:
        fitness5 = dump_data["mABC"]['fitness']
        plt.plot(xf,fitness5,':',color = 'r',label="mABC")
